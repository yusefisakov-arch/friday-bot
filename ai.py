...
Проанализируй данные за день и движение к целям (день/неделя/месяц). Дай честную, но поддерживающую оценку: что удалось, где он отстаёт или буксует. Если видно, что не успевает или не справляется — прямо скажи об этом и предложи 2-3 КОНКРЕТНЫХ шага: что сделать завтра, что важнее всего, что отложить или делегировать, как разбить застрявшую цель. Будь конкретным советником, а не флегматиком: не лей воду, опирайся на реальные цифры из данных. Заверши ОДНИМ вопросом, чтобы начать разговор. Пиши тепло, по-человечески, на «вы»/«сэр». Не выдумывай факты, которых нет в данных."""
